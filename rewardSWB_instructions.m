...
%Added new instruction screens one that looks horrible and one that looks
%'good'

cgpencol(1,1,0)
cgtext('A gamble looks like this. You can imagine each gamble as a coin flip,',-10,150);
cgtext('with different outcomes for Heads or Tails. This gamble represents,',-10,125);
cgtext('a 50% chance of getting each amount',-10,100)
cgtext('This is a practice choice not for real money',-200,-75);
cgtext('Press any mouse button to make a practice choice.',-175,-50);
cgtext('+', 0, 0); cgrect(-10,325,150,150); cgrect(225,-325,150,150);
cgpencol(1,0,1); cgtext('+£0.80', -10,325); cgtext('-£0.40',225,-325);
cgflip(0,0,0)
[~,~,~,mp]=cgmouse; mp=0; while mp==0, [~,~,~,mp]=cgmouse; end;

% Now the 'good' one...

cgpencol(1,0,0); cgtext('A gamble looks like this.',0,300);
cgpencol(0,1,0); cgtext('This gamble represents a 50% chance of getting each amount.',0,250);
cgpencol(0,0,1); cgtext('This is a practice choice not for real money.',0,200);
cgpencol(1,1,1); cgtext('Press any mouse button to make a practice choice.',0,150);
cgtext('+', 0,-125); cgrect(100,-25,150,150); cgrect(100,-225,150,150);
cgpencol(0,0,0); cgtext('+£0.80',100,-25); cgtext('-£0.40',100,-225);
cgpencol(1,1,1); cgrect(-100,-125,150,150); 
cgpencol(0,0,0); cgtext('£0.20',-100,-125);
cgflip(0,0,0)
[~,~,~,mp]=cgmouse; mp=0; while mp==0, [~,~,~,mp]=cgmouse; end;
